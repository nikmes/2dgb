...
void PrintString(int x, int y, bool pixelcoords, unsigned int rgba, const char* str, ...)
{
	...
	if (!pixelcoords)
	{
		...
		x *= (font_width  * dbg_world_size / 1920);
		y *= (font_height * dbg_world_size / 1080);
...
void RenderPlayer()
{
	...
	if (0)//bLaserFire)
	{
		RenderSegment(LaserPos, LaserEndPos, 0xFFFF0000, 0xFF00, 0.0f, 1.0f);
	}
	else
	{
		RenderSegment(LaserPos, LaserEndPos, 0xFFFFFFFF, 0x0000, 10.0f, 1.0f);
	}
}
