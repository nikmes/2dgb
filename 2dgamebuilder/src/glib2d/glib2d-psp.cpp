...
static void* drawBuffer = NULL;
static void* dispBuffer = NULL;
...
void g2dInit()
{
	...
	drawBuffer  = vrelptr(valloc(FRAME_BUFFER_SIZE << 2));
	dispBuffer  = vrelptr(valloc(FRAME_BUFFER_SIZE << 2));
	depthBuffer = vrelptr(valloc(FRAME_BUFFER_SIZE << 1));
	frameBuffer = vabsptr(dispBuffer);

	// Setup GU
	sceGuInit();
	sceGuStart(GU_DIRECT, dlist);

	sceGuDrawBuffer(GU_PSM_8888, drawBuffer, LINE_SIZE);
	sceGuDispBuffer(G2D_SCR_W, G2D_SCR_H, dispBuffer, LINE_SIZE);
	sceGuDepthBuffer(depthBuffer, LINE_SIZE);
...
g2dTexture*g2dTexCreate(int w, int h)
{
	...
	tex->data = (g2dColor*)malloc(tex->tw * tex->th * sizeof(g2dColor));
	if (tex->data == NULL)
	{
		free(tex);
		return NULL;
